...
//		hmvo.setHm_pw(encodedPass);
		...
		MemberVO pwvo = aList.get(0);
		String pw_empnum = pwvo.getHm_empnum();
		
		hmvo.setHm_empnum(pw_empnum);
		List<MemberVO> pwCheckList = etcService.pwCheck(hmvo);
		
		MemberVO pmvo=pwCheckList.get(0);
		String encodedpw=pmvo.getHm_pw();
		
		int nCnt = aList.size();
		boolean pwFlag=false;
		pwFlag=bcpe.matches(hmvo.getHm_pw(), encodedpw);
...
//		//System.out.println(" main IN ETC CONTROLLER >>> " + main);
	...
	public String registerMember(@ModelAttribute ApprVO param, HttpServletRequest request,Model model){
		logger.info("/registerMember() 시작");
		...
		//개인 개발시 사용한 이미지파일 저장 경로.
		boolean flag=false;
		String url="";
		param=humanService.chaebunMemberAppr();//임시 사원번호 부여 
		...
		try{
			...
			logger.info("bFlag 사진 인서트 결과 :  >>> : " + bFlag );
			
				Enumeration<String> en = fuu.getFileNames();
//				dbFileName="../"+"upload"+"/"+fileName; 개인 개발시 사용한 사진파일 저장 경로.
				String hmp_picture =  en.nextElement();
				logger.info("hmp_picture MultipartRequest.getFileNames >>> : " + hmp_picture);
	...
	public String emailAuth(@RequestParam("selectFunc") String selectFunc,
		...
		String toEmail = "";
		String fromEmail = "cdhhn12@gmail.com";
