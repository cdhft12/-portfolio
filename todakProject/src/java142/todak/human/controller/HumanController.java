...
import java142.todak.common.FileDelete;
...
@RequestMapping(value="/human")
	public class HumanController {
		...
		final static String Chaenum="B"; 
		final static String FILEPATH="//home//ec2-user//tomcatt//webapps//todakProject//upload//human";
		@Autowired
	...
	public String  selectMemberAppr(@ModelAttribute ApprVO avo,Model model,HttpServletRequest request)	{
		...
		String cPage=request.getParameter("curPage");//페이징 현재페이지 정보 받아옴
		String pageCtrl=request.getParameter("pageCtrl");//페이징 한번에 표시할 게시물 갯수 정보 받아옴		
		Paging.setPage(avo,cPage,pageCtrl);//받아온 정보를 이용해서 페이징 정보 셋팅	
		...
		if(apprList.size()>0){
			...
			logger.info("조회된 정보 없음!");
		}
		model.addAttribute("apprList",apprList);
		model.addAttribute("avo",avo);
		logger.info("selectApptr 종료");
	...
	public String resultMemberAppr(@ModelAttribute MemberVO mvo,Model model,HttpServletRequest request){
		logger.info("resultMemberAppr 진입");
		...
		if(flag.equals("Y")){
			...
			logger.info("resultMemberAppr 회원가입 승인");
		}
		if(insertFlag){
		...
		if(flag.equals("N")){
			...
			logger.info("resultMemberAppr 회원가입거부");
		}
		logger.info("resultMemberAppr 종료");
		model.addAttribute("flag",insertFlag);
		return "/human/apprResult";
	}
	...
	public String updateMember(@ModelAttribute ApptVO apvo,Model model,HttpSession session){
		...
		logger.info("/updateAppt 종료");
		return "/human/apptResult";
	}
	...
	public boolean authorityCheck(@ModelAttribute MemberVO mvo,HttpSession session){
		...
		logger.info("/deptAuthority 종료");
		return result;
	}
	@ResponseBody
	@RequestMapping(value="idCheckCtr")//id 중복체크
	...
	@ResponseBody
	@RequestMapping(value="/Authority",method = RequestMethod.POST)//회원가입 승인,인사발령  권한 조회
	...
	@RequestMapping("/humanBoard")//게시판 목록 불러오기
	public String humanBoard(@ModelAttribute MemberVO userInfo, Model model,HttpServletRequest request){
		logger.info("게시판 목록 불러오기 humanBoard.td 시작");
	
		LoginSession sManager = LoginSession.getInstance();
	    String sessionId = request.getSession().getId();
	    String hm_empnum = sManager.getUserID(sessionId);  
	  
	    //현재 쿼리에서 인사팀일경우 전부 조회할수있는 기능이 적용 안되고 있음
	  ...
	  //페이징처리 시작
		...
		String ser="";
		 ...
		 if(userInfo.getSearch()==null){
				key=request.getParameter("key");
				ser=request.getParameter("ser");
				
			}else{
				key=userInfo.getKeyword();
				ser=userInfo.getSearch();
			}
			
	    userInfo.setHm_empnum(hm_empnum);
	    userInfo=humanService.selectPosition(userInfo);
	   
	    userInfo.setKeyword(key);
	    userInfo.setSearch(ser);
	    
		Paging.setPage(userInfo,cPage,pageCtrl);
		/*logger.info("pageSize"+userInfo.getPageSize());
		logger.info("groupSize"+userInfo.getGroupSize());
		logger.info("curPage"+userInfo.getCurPage());
		logger.info("totalCount"+userInfo.getTotalCount());
		logger.info("getKeyword"+userInfo.getKeyword());
		logger.info("getSearch"+userInfo.getSearch());*/
	    List<HboardVO> bList=null;  
	    bList=humanService.selectHboard(userInfo);
	    totalCnt=bList.get(0).getTotalCount();
	    
	    System.out.println("totalCnt"+totalCnt);
	    userInfo.setTotalCount(totalCnt);
		
		model.addAttribute("userInfo",userInfo);
	    model.addAttribute("bList",bList);
	    
	    logger.info("게시판 목록 불러오기 humanBoard.td 끝");
		return "human/humanBoard";
	}
	@ResponseBody
	@RequestMapping("boardAuthority")//게시글 열람 자격 확인
	public boolean boardAuthority(@ModelAttribute HboardVO hvo,HttpSession session,HttpServletRequest request){
		logger.info("boardAuthority 시작");
		boolean result=false;
		LoginSession sManager = LoginSession.getInstance();
		String hm_empnum=sManager.getUserID(session.getId());//세션 불러와서 로그인한 아이디의 사번 불러옴
		
		String bnum=hvo.getHb_num();
		
		
		hvo=humanService.selectPost(hvo);
		
		
		MemberVO mvo = new MemberVO();
		mvo.setHm_empnum(hm_empnum);	
		mvo=humanService.selectPosition(mvo);
		if(mvo.getHm_position().equals("15")||mvo.getHm_position().equals("14")||mvo.getHm_teamnum().equals("04")||hvo.getHb_deptnum().equals("98")||hvo.getHb_accepte().equals("Y")){
			result=true;
			
		}
		logger.info("boardAuthority 끝");
		return result;
	}
	@RequestMapping("boardDetail")//게시판 상세페이지 처리 컨트롤러
	public String boardDetail(@ModelAttribute HboardVO hvo, Model model,HttpSession session,HttpServletRequest request){
		logger.info("boardDetail() 시작");
		String url="";
		logger.info("출력할 게시글 번호 확인:"+hvo.getHb_num());
	    ...
	    mvo.setHm_empnum(hm_empnum);
	    mvo=humanService.selectPosition(mvo);
	    hitFlag=humanService.boardHit(hvo);
	    
	    if(hitFlag){
	    	logger.info("조회수 hit 성공");
	    }else{
	    	logger.info("조회수 hit 실패");
	    }
	    
		List<HboardVO> boardList= humanService.boardDetail(hvo);
		
		model.addAttribute("boardList",boardList);
		model.addAttribute("userInfo",mvo);
		url="human/boardDetail";
		if(boardList.size()==0){
			logger.info("리스트 사이즈 0");
		}
		logger.info("boardDetail() 끝");
		return url;
	}
	@RequestMapping("boardAccept")
	public String boardAccept(@ModelAttribute HboardVO hvo,Model model){
		boolean flag=false;
		
		flag=humanService.boardAccept(hvo);
		String url="";
		
		if(flag){
			url="/human/humanBoard.td";
		}
		return "redirect:"+url;
	}
	@RequestMapping("/boardDownload")//다운로드
	public void boardDownload(@ModelAttribute HboardVO hvo,HttpServletResponse response,HttpServletRequest request){
		String filePath=request.getSession().getServletContext().getRealPath("/upload/human");
		String fileName=hvo.getHb_file();
		
		File file = new File(filePath,fileName);//파일 경로뿐만 아니라 파일이름도 같이 지정해줘야   java.io.FileNotFoundException: (액세스가 거부되었습니다) 발생안함
		...
		try{
			...
			if(browser.contains("MSIE")||browser.contains("Trident")||browser.contains("Chrome")){
				dName=URLEncoder.encode(fileName,"UTF-8").replaceAll("\\+", "%20");
			}else{
				dName = new String(fileName.getBytes("UTF-8"), "ISO-8859-1");
			}
	...
	@RequestMapping("moveInsert")//인사정보 출력해서 게시글 작성페이지로 이동
	public String moveInsert(@ModelAttribute MemberVO mvo, Model model,HttpServletRequest request){
		LoginSession sManager = LoginSession.getInstance();
	    String sessionId = request.getSession().getId();
	    String hm_empnum = sManager.getUserID(sessionId);
	    mvo.setHm_empnum(hm_empnum);
		...
		return "human/insertHboard";
	}
	@RequestMapping("/insertHboard")//게시글 작성 컨트롤러
	public String insertHboard(@ModelAttribute HboardVO hvo, Model model,HttpServletRequest request){
        ...
        String fPath=request.getSession().getServletContext().getRealPath("/upload/human");
		...
		ChaeNo=humanService.chaebunBoard();
		ChaeNo=ChaebunUtils.cNum(ChaeNo, Chaenum);
		hvo.setHb_num(ChaeNo);

		System.out.println(hvo.getHb_num());
		
		FileUploadUtil fuu = new FileUploadUtil();
		bFlag = fuu.fileUpload(request, fPath);
		
		 if(bFlag){	
			
			 Enumeration<String> en = fuu.getFileNames();
			 firstFile = en.nextElement();
			 secondFile = en.nextElement();
			
			 if(firstFile==null && secondFile!=null){
				 hvo.setHb_image(secondFile);
				 hvo.setHb_file("");
			 }
			 if(firstFile!=null && secondFile==null){
				 hvo.setHb_file(firstFile);
				 hvo.setHb_image("");
			 }
			 if(firstFile!=null && secondFile!=null){
				 hvo.setHb_image(secondFile);
			 ...
			 System.out.println("2"+hvo.getHb_image());
			 String hb_title=fuu.getParameter("hb_title");
			 String hb_content=fuu.getParameter("hb_content");
			 String hb_deptnum=fuu.getParameter("hb_deptnum");
			 ...
			 if(hb_deptnum.equals("98")){
				 hb_accepte="Y";
			 }else{
				 hb_accepte="N";
			 }
			 
			 hvo.setHb_title(hb_title);
			 hvo.setHb_content(hb_content);
			 hvo.setHb_deptnum(hb_deptnum);
			 hvo.setHm_name(hm_name);
			 hvo.setHm_position(hm_position);
			 hvo.setHm_duty(hm_duty);
			 hvo.setHm_empnum(hm_empnum);
			 hvo.setHb_accepte(hb_accepte);
			
			  hvo.setHb_accepte(hb_accepte);
			  System.out.println(hvo.getHb_num());
			  System.out.println(hvo.getHm_empnum());
			  System.out.println(hvo.getHm_name());
			  System.out.println(hvo.getHb_deptnum());
			  System.out.println(hvo.getHb_title());
			  System.out.println(hvo.getHb_content());
			  System.out.println(hvo.getHb_image());
			  System.out.println(hvo.getHb_file());
			  System.out.println(hvo.getHm_position());
			  System.out.println(hvo.getHm_duty());
			  System.out.println(hvo.getHb_accepte());
		
			  
			  
		
			 sqlFlag=humanService.insertHboard(hvo);
		...
		logger.info("insertHboard 끝");
		return "redirect:"+url;//업데이트 성공했을씨.
	}
	@RequestMapping("moveUpdate")//인사정보 출력해서 게시글 작성페이지로 이동
	public String moveUpdate(@ModelAttribute HboardVO hvo, Model model){
		
		logger.info("moveUpdate 시작");
		
		
		model.addAttribute("hvo", hvo);
		
		logger.info("moveUpdate 끝");
		
		return "human/updateHboard";
		
	}
	@RequestMapping("/updateHboard")//게시글 작성 컨트롤러
	public String updateHboard(@ModelAttribute HboardVO hvo, Model model,HttpServletRequest request){
		...
		String imgFlag=fuu.getParameter("imgFlag");
		String fileFlag=fuu.getParameter("fileFlag");
		
		String imgDivFlag=fuu.getParameter("imgDivFlag");
		String fileDivFlag=fuu.getParameter("fileDivFlag");
		
		String hb_num=fuu.getParameter("hb_num");
		
		String olderImg="";
		String olderFile="";
		
		
		HboardVO updateVO= new HboardVO();

		updateVO.setHb_num(hb_num);		
	
		updateVO=humanService.uploadSelect(updateVO);//DB에 저장되어 있는 img 첨부파일 이름 불러오는 쿼리 실행. 
		
		if((!imgFlag.equals("delete"))&&(!imgDivFlag.equals("null"))){
			olderImg=updateVO.getHb_image();
		}
		if((!fileFlag.equals("delete"))&&(!fileDivFlag.equals("null"))){
			olderFile=updateVO.getHb_file();
		}
		
		FileDelete fd= new FileDelete();
		
		if(imgFlag.equals("delete")){
			olderImg=updateVO.getHb_image();
			deleteFlag=fd.deleteFile(olderImg, fPath);
			if(deleteFlag){
				logger.info("이미지 파일 삭제 완료!");
			}else{
				logger.info("이미지 파일 삭제 실패!");
			}
			olderImg="";
		}
		if(fileFlag.equals("delete")){
			olderFile=updateVO.getHb_file();
			deleteFlag=fd.deleteFile(olderFile, fPath);
			if(deleteFlag){
				logger.info("첨부 파일 삭제 완료!");
			}else{
				logger.info("첨부 파일 삭제 실패!");
			}
			olderFile="";
		}
		 ...
		 if(bFlag){	
			 ...
			 if(firstFile==null && secondFile!=null){
				 hvo.setHb_image(secondFile);
				 hvo.setHb_file(olderFile);				 
			 }
			 if(firstFile!=null && secondFile==null){
				 hvo.setHb_file(firstFile);
				 hvo.setHb_image(olderImg);
			 }
			 if(firstFile!=null && secondFile!=null){
				 hvo.setHb_image(secondFile);
				 hvo.setHb_file(firstFile);
			 ...
			 hvo.setHb_num(hb_num);
			  
			  
		
			 sqlFlag=humanService.updateHboard(hvo);
